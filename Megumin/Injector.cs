...
namespace Megumin
{
    partial class MainWindow
    {
        ...
        [DllImport("kernel32.dll")]
        public static extern bool WriteProcessMemory(IntPtr hProcess, IntPtr lpBaseAddress, char[] lpBuffer, int nSize,
        ...
        private void Inject(string path)
        {
            ...
            if (processes.Length == 0)
            {
                ...
                if (Interaction.Shell("explorer.exe shell:appsFolder\\Microsoft.MinecraftUWP_8wekyb3d8bbwe!App",
                        Wait: false) == 0)
                {
            ...
            var handle = OpenProcess((IntPtr) 2035711, false, (uint) process.Id);
            ...
            var p1 = VirtualAllocEx(handle, IntPtr.Zero, (uint) (path.Length + 1), 12288U, 64U);
            WriteProcessMemory(handle, p1, path.ToCharArray(), path.Length, out var p2);
            var procAddress = GetProcAddress(GetModuleHandle("kernel32.dll"), "LoadLibraryA");
            var p3 = CreateRemoteThread(handle, IntPtr.Zero, 0U, procAddress, p1, 0U, ref p2);
            if (p3 == IntPtr.Zero)
            {
            ...
            var n = WaitForSingleObject(p3, 5000);
            if (n == 128L || n == 258L)
            {
                CloseHandle(p3);
            }
            ...
            var windowH = FindWindow(null, "Minecraft");
