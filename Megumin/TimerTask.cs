...
namespace IKriv.Threading.Tasks
{
    ...
    /// </summary>
    /// <remarks>
    ///     Task timer must be disposed (stopped) in the end of the usage.
    ...
    /// </summary>
    /// <remarks>
    ///     Use TaskTimer to perform timer-based activities in asynchronous code.
    ///     Await on each generated task in order.
    /// </remarks>
    public class TaskTimer
    {
        ...
        /// <returns>
        ///     This overload is used for tests and custom scenarios. The object created by createTimer()
        ...
        private class TaskTimerImpl : IDisposable
        {
            ...
            private void OnTimer()
            {
                ...
                foreach (var promise in toComplete) promise.SetResult(default);
            }

            public IEnumerable<Task> GetTasks()
            {
                while (true) yield return NextTask();
            ...
            private Task NextTask()
            {
                ...
                {
                    var tick = ++_nTasks;
                    var isLate = tick <= _ticks;

                    if (_isCanceled) return CreateCanceledTask();

                    if (isLate) return Task.FromResult(Void.Value);
            ...
            private void Cancel()
            {
                ...
                {
                    _isCanceled = true;
                    foreach (var record in _pendingTasks) record.Promise.TrySetCanceled();
                }
            }
